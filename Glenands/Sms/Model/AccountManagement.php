...
class AccountManagement implements AccountManagementInterface
{
     ...
     */
    public function checkPasswordStrengthGlenands($password)
    {
     ...
     * @SuppressWarnings(PHPMD.NPathComplexity)
     */
    public function createAccountWithPasswordHashCustom(CustomerInterface $customer, $hash, $redirectUrl = '', $phoneNumber = '')
    {
        ...
        try {
            ...
            //$custom['custom_attributes'] = ['attribute_code' => 'phone_number', 'value' => $phoneNumber];
            //$customer->setData('custom_attributes', $custom['custom_attributes']);
            //   print_r($customer->__toArray());die();
    ...
    public function initiatePasswordReset($email, $template, $websiteId = null)
    {
        ...
        try {
            if($customer->getCustomAttribute('phone_number')){
                ...
                $url = $this->storeManager->getStore()->getBaseUrl()."/customer/account/createpassword?id=".$customer->getId()."&token=".$newPasswordToken;
                $response = $this->sms->sendForgotPasswordLink($customer->getCustomAttribute('phone_number')->getValue(), $url);
            }
            switch ($template) {
     ...
     * @throws LocalizedException
     */
    public function createAccountCustom(CustomerInterface $customer, $password = null, $redirectUrl = '', $phoneNumber = '')
    {
        if ($password !== null) {
            $this->checkPasswordStrength($password);
            $customerEmail = $customer->getEmail();
        ...
        return $this->createAccountWithPasswordHashCustom($customer, $hash, $redirectUrl, $phoneNumber);
    }
