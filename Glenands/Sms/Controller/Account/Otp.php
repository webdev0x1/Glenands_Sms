 ...
 */ 
class Otp extends \Magento\Framework\App\Action\Action implements HttpGetActionInterface, HttpPostActionInterface
{
    /**
    ...
    public function execute()
    {
        ...
        if(!empty($hashLogin)) {
            ...
            if($customer->getCustomAttribute('phone_number') == null) {
                /** @var \Magento\Framework\View\Result\Page $resultPage */
                $resultPage = $this->resultPageFactory->create();
                $resultPage->getConfig()->getTitle()->set((__('Enter Phone number')));
                $resultPage->setHeader('Phone-Number-Required', 'true');
                return $resultPage;  
            }
        }
        if($sms && empty($this->request->getParam('otp')) && !$sms->getData('otp_text')) {
            $message = 'There is a problem with generating OTP! Please try login again!';
            ...
            if(!empty($sms) && $sms->getData('hashlogin') == $hashLogin) {
                ...
                $resultPage->getConfig()->getTitle()->set((__('Enter OTP')));
                return $resultPage;
            } else {
